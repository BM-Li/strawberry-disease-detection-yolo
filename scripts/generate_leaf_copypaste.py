...
DEFAULT_DATASET = ROOT / "datasets" / "the_big"
DEFAULT_BG_DIR = ROOT / "datasets" / "bg_for_leafspot"
IMAGE_EXTS = {".jpg", ".jpeg", ".png", ".bmp", ".tif", ".tiff", ".webp"}

# Default prefixes per class
CLASS_PREFIXES = {0: "cp_angular", 3: "cp_leaf"}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate synthetic Leaf Spot / Angular Leafspot samples with copy-paste augmentation."
    )
    parser.add_argument(
        "--dataset",
        type=Path,
        default=DEFAULT_DATASET,
        help="YOLO dataset root. Default: datasets/the_big",
    )
    parser.add_argument(
        "--split",
        type=str,
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="Splits used to extract patches. Default: same as --split",
    )
    parser.add_argument(
        "--class-id",
        type=int,
        default=3,
        choices=[0, 3],
        help="Target class id. 0=Angular Leafspot, 3=Leaf Spot. Default: 3",
    )
    parser.add_argument(
        "--num-outputs",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=24,
        help="Minimum non-zero mask area in pixels. Default: 24",
    )
    parser.add_argument(
        "--prefix",
        type=str,
        default=None,
        help="Filename prefix for synthetic outputs. Default: cp_angular (class 0) or cp_leaf (class 3)",
    )
    parser.add_argument(
        "--seed",
        type=int,
    ...
    parser.add_argument(
        "--custom-bg-dir",
        type=Path,
        default=DEFAULT_BG_DIR,
        help="Directory of leaf background images. Default: datasets/bg_for_leafspot",
    )
...
def compute_lesion_mask(crop: np.ndarray, min_mask_area: int) -> np.ndarray:
    """Extract lesion mask for leaf spot / angular leafspot.

    Uses more relaxed HSV thresholds compared to anthracnose because leaf spot
    lesions are brownish/yellowish (lighter) rather than dark.
      - dark_thresh: 45th percentile of V channel (vs. 55th for anthracnose)
      - sat_thresh : 60th percentile of S channel (vs. 70th for anthracnose)
    """
    ...
    # Relaxed thresholds for brownish/yellowish leaf spot lesions
    dark_thresh = max(25, int(np.percentile(val, 45)))
    sat_thresh = int(np.percentile(sat, 60))
...
def generate(
    ...
    """Generate synthetic leaf disease images via copy-paste augmentation.

    Public interface matches generate_anthracnose_copypaste.generate() for
    ...
    class_name = {0: "Angular Leafspot", 3: "Leaf Spot"}.get(class_id, f"class {class_id}")

    print("=" * 72)
    print(f"Leaf Disease Copy-Paste Augmentation — {class_name}")
    ...
    if not patches:
        raise RuntimeError(
            f"No patches extracted for class {class_id}. "
            "Check labels and class id."
        )
    if not backgrounds:
        raise RuntimeError(
            "No backgrounds found. Provide custom_bg_dir (bg_for_leafspot) or "
            "ensure the target split contains unlabeled / empty-label background images."
        )
...
def generate_leaf_diseases(
    dataset_dir: Path,
    class_ids: list[int],
    split: str = "train",
    source_splits: list[str] | None = None,
    num_outputs: int | dict[int, int] = 100,
    max_patches_per_image: int = 3,
    min_patch_size: int = 14,
    min_mask_area: int = 24,
    seed: int = 42,
    dry_run: bool = False,
    custom_bg_dir: str | Path | None = None,
    reset_prefix: bool = False,
) -> None:
    """Convenience wrapper: generate synthetic images for multiple leaf disease classes.

    Each class_id is processed independently with its default prefix
    (cp_angular for class 0, cp_leaf for class 3).

    Args:
        class_ids: List of class IDs to process, e.g. [0, 3].
        num_outputs: Either a single int applied to all classes, or a dict
                     mapping class_id → number of outputs.
    """
    _src = source_splits or [split]
    for class_id in class_ids:
        prefix = CLASS_PREFIXES.get(class_id, f"cp_class{class_id}")
        n = num_outputs.get(class_id, 100) if isinstance(num_outputs, dict) else num_outputs
        generate(
            dataset_dir=dataset_dir,
            split=split,
            source_splits=_src,
...
def main() -> None:
    ...
    prefix = args.prefix or CLASS_PREFIXES.get(args.class_id, f"cp_class{args.class_id}")
    generate(
